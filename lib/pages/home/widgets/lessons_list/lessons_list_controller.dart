...
class LessonsListController extends GetxController {
  ...
  Future<void> openLessonPage({required Lesson lesson, required Color color}) async {
    await switchBackgroundMusicState(false);
    await Get.toNamed(
      AppRoutes.lesson,
      arguments: LessonPageArguments(lesson: lesson, color: color),
    );
    await switchBackgroundMusicState(true);
  }

  Future<void> switchBackgroundMusicState(bool play) async {
    ...
    await (play ? homePageController?.playbackgroundMusic() : homePageController?.pauseBackgroundMusic());
  }

  Future<void> openPuzzlePage() async {
    await switchBackgroundMusicState(false);
    await Get.toNamed(AppRoutes.puzzle);
    await switchBackgroundMusicState(true);
  }
